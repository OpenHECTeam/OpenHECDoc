...
2. ##### **SRAM初始化复位**
   ...
   SRAM复位操作。设置复位1, 点击单步时钟多次后，对SRAM进行复位；设置复位为0，点击单步时钟多次后，SRAM初始化完毕。

3. ##### 写存储

   第一步：位流配置成功后，点击**高级设置**中的**SRAM访问设置**，选择**打开WEB模式访问SRAM存储**，返回成功信息后；然后点击写存储，展开写存储操作页面。

   第二步：选择SRAM的基地址0x5000000，WEB下访问存储的地址范围如下表所示。
   ...
   第三步：增加值或者导入写入的文件，导入文件的格式为二进制文件编码；从ROM基地址依次写入1, 2, … , f的值。![](/assets/writeram0002.png)

   第四步： 点击发送数据，此时才会开始把准备好的数据，写入到教学板的SRAM中。
...
4. ##### 读存储
   ...
   第三步：点击查看结果，在下方表中可以SRAM中的数据，也可以生成文件导出。

   第四步：点击**高级设置**中的**SRAM访问设置**，选择关闭WEB模式访问SRAM存储。
...
7. **友情提示**
   ...
   在使用WEB页面的SRAM读存储和写存储时，需要打开WEB模式访问SRAM存储；不使用时需要关闭WEB模式访问SRAM存储。
