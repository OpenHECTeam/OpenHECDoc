...
|  一级目录  |  二级目录  |  说明  |
|  :-----  | :-----    |  :--:  |
| base_ip  | 	       |  存放可直接调用的基础IP核包  |
| bit	   |           |  存放生成的位流文件           |
| dcp      | relax_syn |  OpenHEC平台预先生成好的实验支撑包dcp文件  |
|          | user_syn  |  用户逻辑部分综合后生成的dcp文件   |
|          | user_imp |   带上实验支撑包的用户逻辑实现后生成的dcp文件  |
| doc      |          |   存放使用文档  |
| proj_ip  | 		  |   用户逻辑的vivado工程路径  |
| relax_top| 		  |   OpenHEC平台的顶层文件  |
| tcl      |          |   工程项目进行综合与实现所需用到的tcl脚本文件  |

二、实验支撑包**lab_top**接口说明
实验顶层模块lab_top中留出来的接口有FPGA自激励时钟、单步时钟、复位信号、通信IO输入输出信号、一片ROM接口和两片RAM读写接口。具体的信号接口定义如下表所示。
